...
int main()
{
    ...
    mqttClient->publishMessageAsync("Log/Highbay/SubTopics", "subscribed", 2);
    ...
    while (true)
    {
        ...
        if (orderFlag)
        {
            mqttClient->publishMessageAsync("Log/Highbay/OrderFlag", "True", 2);
        }
        else
        {
            mqttClient->publishMessageAsync("Log/Highbay/OrderFlag", "False", 2);
        }
        // ToDo: remove sleep()
        // sleep(500ms);
    }
